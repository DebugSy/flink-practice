 ...
 *  }
 */
public class FlinkSqlTraining_join_row {

    /**
     * 两个流基于时间窗口的join
     */
    private static String innerJoinWithTimeWindowSql = "insert into sink_console " +
            "SELECT c.userId,c.username,url,clickTime,data_col,time_col,address,activityTime " +
    ...
    public static TypeInformation JOIN_RESULT = Types.ROW(
            new String[]{"userId", "username", "url", "clickTime", "data_col", "time_col", "address", "activityTime"},
            new TypeInformation[]{
                    Types.INT(),
                    Types.STRING(),
                    Types.STRING(),
